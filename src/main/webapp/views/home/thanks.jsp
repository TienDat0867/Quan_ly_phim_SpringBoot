     ...
     <section class="movie-detail">
 			 <div class="container" style="padding:50px">
			     ...
			     <div class="ticket">
			     	...
			     	<p>Tên phim :${sessionScope.phimChoose.tenphim } </p>
			     	<p>Suất Chiếu :  ${sessionScope.giochieu}  -  ${sessionScope.ngaychieu}</p>
			     	<p>Ghế : 
			     		<c:forEach items="${sessionScope.ls_seat}" var="seat" varStatus="status">
					                    ${seat}<c:if test="${!status.last}">, </c:if>
					     </c:forEach>
			     	</p>
			     	<p>Số Tiền Thanh Toán :${totalPrice} VND </p>
			     	<p>Mã Giao Dịch: ${transactionId} </p>
			     	<p>Trạng Thái Thanh Toán : ${status }</p>
			     	<p>Thông Tin Chi tiết giao dịch sẽ được gửi trong Email : </p>
			     </div>
			     </div>
